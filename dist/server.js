...
let discordRest = null;
if (discordToken) {
    discordRest = new rest_1.REST({ version: '10' }).setToken(discordToken);
...
app.post('/api/auth/login', async (req, res) => {
    ...
    try {
        ...
        const token = jsonwebtoken_1.default.sign({ id: user.id, email: user.email }, JWT_SECRET, { expiresIn: '12h' });
        res.json({ token });
    }
    catch (error) {
...
app.get('/api/status', authenticateToken, async (req, res) => {
    ...
    try {
        await discordRest.get(discord_js_1.Routes.user('@me'));
...
app.post('/api/config/instructions', authenticateToken, async (req, res) => {
    try {
        ...
        if (channelId) {
            if (instructions) {
                await pool.query('INSERT INTO channel_instructions (channel_id, instructions) VALUES ($1, $2) ON CONFLICT (channel_id) DO UPDATE SET instructions = EXCLUDED.instructions', [channelId, instructions]);
            }
            else {
...
app.get('/api/channels', authenticateToken, async (req, res) => {
    try {
        ...
        const channelsWithNames = await Promise.all(channels.map(async (id) => {
            ...
            if (discordRest) {
                try {
                    const channel = await discordRest.get(discord_js_1.Routes.channel(id));
...
app.post('/api/channels', authenticateToken, async (req, res) => {
    try {
        ...
        if (discordRest) {
            try {
                await discordRest.get(discord_js_1.Routes.channel(channelId));
            }
            catch (e) {
                // Warn or fail?
            }
...
app.post('/api/upload', authenticateToken, upload.single('file'), async (req, res) => {
    ...
    */
    try {
        ...
        await pool.query('INSERT INTO knowledge_base (filename, content, upload_date) VALUES ($1, $2, $3)', [req.file.originalname, data.text, Date.now()]);
        // Cleanup temp file
